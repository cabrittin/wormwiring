...
ImporterList = function(parent,list)
{
    var ul = document.createElement('ul')
    if (list.id != undefined && list.id != null){
	ul.id = list.id;
    };
    if (list.class != undefined && list.class != null){
	ul.className = list.className
    };
    if (list.elements != undefined && list.elements != null){
	for(var i in list.elements){
	    if (list.elements[i] != ""){
		var li = document.createElement('li');
		li.innerHTML = list.elements[i];
		ul.appendChild(li);
	    };
	};
    };
...
ImporterMenu.prototype.AddRadioSelect = function(parent,params)
{
    var selectorForm = document.createElement('form');
    selectorForm.id = params.id;
    if (params.options != undefined && params.options != null){
	for (var i=0;i<params.options.length;i++){
	    var label = document.createElement('label');
	    var input = document.createElement('input');
	    input.type='radio';
	    input.value = params.options[i].value;
	    //input.innerHTML = params.options[i].text;
	    input.name = params.name;
	    if (params.options[i].checked){
		input.setAttribute('checked','checked');
	    };
	    label.innerHTML = params.options[i].text;
	    selectorForm.appendChild(input);
	    selectorForm.appendChild(label);
	    var br = document.createElement('br');
	    selectorForm.appendChild(br)
	};
    };
    if (params.onChange != undefined && params.onChange != null){
	selectorForm.onchange = params.onChange;
    };
    parent.appendChild(selectorForm);
}
